...
function PhotoSlot({ label, hint, image, required, inputRef, onFile, onClear }) {
  return (
    <div className="flex flex-col items-center gap-2.5 rounded-md border border-dashed border-border bg-panel p-4 text-center">
      <div className="text-[12.5px] font-semibold text-ink">
        {label}{required ? "" : <span className="ml-1 font-normal text-ink-soft">(optional)</span>}
      </div>
      {image ? (
        <div className="relative w-full">
          <img src={image.dataUrl} alt={`${label} of the package`} className="h-28 w-full rounded border border-border object-cover" />
          <button
            type="button"
            onClick={onClear}
...
export function ScanView({ onSave, onUpdateScan, ruleVersions, inspectorName = "Inspector on duty" }) {
  ...
  // 4 pre-loaded demo products in front of the camera for the pitch-table
  ...
  const [scanMode, setScanMode] = useState("label"); // label | object
  ...
  const [objectScanSource, setObjectScanSource] = useState("device"); // device | phone
  ...
  function handleDemoProduct(product) {
    ...
    setImage({ dataUrl: null, base64: null, mediaType: null, hashHex: demoHash(product.match) });
    setCaptureMeta({
      gps: GPS_BY_REGION[saveForm.region],
      timestamp: new Date().toISOString(),
      nonce: generateNonce(),
    ...
    <div className="lm-panel">
      ...
      {phase === "idle" && (
        <div className="flex flex-col gap-6">
          <div className="inline-flex w-fit rounded-md border border-border bg-panel-alt p-1">
            <button
              type="button"
              onClick={() => setScanMode("label")}
              className={"rounded px-3.5 py-1.5 text-[13px] font-semibold transition-colors " + (scanMode === "label" ? "bg-panel text-ink shadow-sm" : "text-ink-soft")}
            >
              Scan a label
            </button>
            <button
              ...
              className={"rounded px-3.5 py-1.5 text-[13px] font-semibold transition-colors " + (scanMode === "object" ? "bg-panel text-ink shadow-sm" : "text-ink-soft")}
            >
              Scan a 3D object
            </button>
          </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="inline-flex w-fit rounded-md border border-border bg-panel p-1">
                <button
                  ...
                  className={"rounded px-3 py-1 text-[12.5px] font-semibold transition-colors " + (objectScanSource === "device" ? "bg-panel-alt text-ink shadow-sm" : "text-ink-soft")}
                >
                ...
                <button
                  ...
                  className={"rounded px-3 py-1 text-[12.5px] font-semibold transition-colors " + (objectScanSource === "phone" ? "bg-panel-alt text-ink shadow-sm" : "text-ink-soft")}
                >
                  Scan with phone
                </button>
              </div>
      ...
      {phase === "analyzing" && (
        <div className="flex flex-col gap-3">
          {image && <img src={image.dataUrl} alt="Label being analyzed" className="mx-auto max-w-[320px] rounded border border-border" />}
      ...
      {phase === "result" && result && (() => {
        ...
        return (
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-md border border-border bg-panel-alt p-4">
              ...
              {scaleChecked && declaredQty && (
                <div className={"mt-2.5 flex items-center gap-1.5 rounded px-2.5 py-1.5 text-[12.5px] " + (withinTolerance ? "bg-green-soft text-green" : "bg-brass-soft text-navy-deep")}>
                  {withinTolerance
            ...
            {violationDecision && violationDecision.type === "disputed" && (
              <div className="flex items-center gap-1.5 rounded bg-brass-soft px-2.5 py-1.5 text-[13px] text-navy-deep">
