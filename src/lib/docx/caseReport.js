...
export async function downloadReportDocx(scan) {
  const children = [
    ...
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: "\tDate: " + formatDate(new Date().toISOString().slice(0, 10)), size: 17, color: HEX.inkSoft }),
      ],
    }),
  ...
  children.push(
    ...
    body("The image captured for this inspection was hashed at the time of scan using SHA-256. Any subsequent alteration of the image file would produce a different hash value, providing a basic tamper-evidence check for this record.", { spaceAfter: 120 }),
    new Paragraph({
      shading: { type: ShadingType.CLEAR, fill: "EFEDE6" },
      spacing: { after: 40 },
      children: [new TextRun({ text: "SHA-256:  " + scan.hash, font: "Courier New", size: 17 })],
    }),
    new Paragraph({
      shading: { type: ShadingType.CLEAR, fill: "EFEDE6" },
      spacing: { after: 40 },
